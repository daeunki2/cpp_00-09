...
/*   Updated: 2025/09/11 14:48:23 by daeunki2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::increment()
{
    ...
    grade--;
	std::cout << "Promotion! Bureaucrat " << this->getName() << "is now grade" << this->getGrade() << std::endl;
};
...
//excute
void Bureaucrat::executeForm(const AForm &paper)
{
    try
    {
        paper.execute(*this); 
    ...
    catch (const std::exception& e)
    {
        std::cerr << this->getName() << " couldn’t execute " 
                  << paper.getName() << " because " << e.what() << std::endl;
    }
}
